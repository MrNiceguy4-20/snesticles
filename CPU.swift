...
class CPU {
    ...
    func am_acc() { self.opAddr = 0; self.cyclesRemaining += 1 }
    func am_ind_long() {
    ...
    func am_sr() { self.opAddr = UInt32(self.SP &+ UInt16(self.fetchByte())); self.cyclesRemaining += 1 }
    func am_sr_iy() {
    ...
    func branch(_ c: Bool) {
        ...
        if c {
            ...
            self.cyclesRemaining += 2
            if (self.PC & 0xFF00) != (oldPC & 0xFF00) { self.cyclesRemaining += 1 }
        } else {
            self.cyclesRemaining += 1
        }
    }
    ...
    func op_sbc(_ m: ()->Void) {
        m(); let is8 = self.e_mode || self.P.contains(.m)
        let op = is8 ? UInt16(self.bus.read(self.opAddr)) : self.bus.readWord(self.opAddr)
        let borrow = self.P.contains(.c) ? 0 : 1
    ...
    func op_pea_imm16() { let value = self.fetchWord(); self.pushWord(value); self.cyclesRemaining = 5 }
    func op_pei(_ m: ()->Void) { m(); self.pushWord(UInt16(self.opAddr & 0xFFFF)); self.cyclesRemaining=6 }
    ...
    private func setupOpcodeTable() {
        ...
        self.lookup[0xEA] = { self.cyclesRemaining = 2 }
        self.lookup[0xCB] = { self.waitingForInterrupt = true; self.cyclesRemaining = 2 }
        self.lookup[0x42] = { self.op_wdm() }
        self.lookup[0x00] = { self.op_brk() }
        self.lookup[0x02] = { self.op_cop() }
        self.lookup[0xEB] = { self.op_xba() }

        self.lookup[0x18] = { self.P.remove(.c); self.cyclesRemaining = 2 }
        self.lookup[0x38] = { self.P.insert(.c); self.cyclesRemaining = 2 }
        self.lookup[0x58] = { self.P.remove(.i); self.cyclesRemaining = 2 }
        self.lookup[0x78] = { self.P.insert(.i); self.cyclesRemaining = 2 }
        self.lookup[0xD8] = { self.P.remove(.d); self.cyclesRemaining = 2 }
        self.lookup[0xF8] = { self.P.insert(.d); self.cyclesRemaining = 2 }
        self.lookup[0xFB] = { let c=self.P.contains(.c); let e=self.e_mode; self.e_mode=c; if e {self.P.insert(.c)} else {self.P.remove(.c)}; if !self.e_mode {self.P.insert([.m,.x])}; self.cyclesRemaining=2 }
        self.lookup[0xC2] = { self.op_rep() }
        self.lookup[0xE2] = { self.op_sep() }

        self.lookup[0xA9] = { self.op_lda(self.am_imm_m) }
        self.lookup[0xA5] = { self.op_lda(self.am_dp) }
        self.lookup[0xA7] = { self.op_lda(self.am_ind_long_dp) }
        self.lookup[0xAD] = { self.op_lda(self.am_abs) }
        self.lookup[0xAF] = { self.op_lda(self.am_abs_long) }
        self.lookup[0xB1] = { self.op_lda(self.am_dp_ind_y) }
        self.lookup[0xB2] = { self.op_lda(self.am_ind) }
        self.lookup[0xB3] = { self.op_lda(self.am_sr_iy) }
        self.lookup[0xB7] = { self.op_lda(self.am_ind_long_dp_y) }
        self.lookup[0xBD] = { self.op_lda(self.am_abs_x) }
        self.lookup[0xBF] = { self.op_lda(self.am_abs_long_x) }
        self.lookup[0xB9] = { self.op_lda(self.am_abs_y) }
        self.lookup[0xB5] = { self.op_lda(self.am_dp_x) }
        self.lookup[0xA1] = { self.op_lda(self.am_dp_x_ind) }
        self.lookup[0xA3] = { self.op_lda(self.am_sr) }

        self.lookup[0x85] = { self.op_sta(self.am_dp) }
        self.lookup[0x87] = { self.op_sta(self.am_ind_long_dp) }
        self.lookup[0x8D] = { self.op_sta(self.am_abs) }
        self.lookup[0x8F] = { self.op_sta(self.am_abs_long) }
        self.lookup[0x91] = { self.op_sta(self.am_dp_ind_y) }
        self.lookup[0x92] = { self.op_sta(self.am_ind) }
        self.lookup[0x93] = { self.op_sta(self.am_sr_iy) }
        self.lookup[0x97] = { self.op_sta(self.am_ind_long_dp_y) }
        self.lookup[0x99] = { self.op_sta(self.am_abs_y) }
        self.lookup[0x9D] = { self.op_sta(self.am_abs_x) }
        self.lookup[0x9F] = { self.op_sta(self.am_abs_long_x) }
        self.lookup[0x81] = { self.op_sta(self.am_dp_x_ind) }
        self.lookup[0x83] = { self.op_sta(self.am_sr) }
        self.lookup[0x95] = { self.op_sta(self.am_dp_x) }

        self.lookup[0xA2] = { self.op_ldx(self.am_imm_x) }
        self.lookup[0xA6] = { self.op_ldx(self.am_dp) }
        self.lookup[0xAE] = { self.op_ldx(self.am_abs) }
        self.lookup[0xBE] = { self.op_ldx(self.am_abs_y) }
        self.lookup[0xB6] = { self.op_ldx(self.am_dp_y) }

        self.lookup[0x86] = { self.op_stx(self.am_dp) }
        self.lookup[0x8E] = { self.op_stx(self.am_abs) }
        self.lookup[0x96] = { self.op_stx(self.am_dp_y) }
        self.lookup[0x9E] = { self.op_stx(self.am_abs_y) }

        self.lookup[0xA0] = { self.op_ldy(self.am_imm_x) }
        self.lookup[0xA4] = { self.op_ldy(self.am_dp) }
        self.lookup[0xAC] = { self.op_ldy(self.am_abs) }
        self.lookup[0xBC] = { self.op_ldy(self.am_abs_x) }
        self.lookup[0xB4] = { self.op_ldy(self.am_dp_x) }

        self.lookup[0x84] = { self.op_sty(self.am_dp) }
        self.lookup[0x8C] = { self.op_sty(self.am_abs) }
        self.lookup[0x94] = { self.op_sty(self.am_dp_x) }

        self.lookup[0x64] = { self.op_stz(self.am_dp) }
        self.lookup[0x74] = { self.op_stz(self.am_dp_x) }
        self.lookup[0x9C] = { self.op_stz(self.am_abs) }
        self.lookup[0x9E] = { self.op_stz(self.am_abs_x) }
        ...
        self.lookup[0x3F] = { self.op_and(self.am_abs_long_x) }
        ...
        self.lookup[0x37] = { self.op_and(self.am_ind_long_dp_y) }
        self.lookup[0x39] = { self.op_and(self.am_abs_y) }
        self.lookup[0x3D] = { self.op_and(self.am_abs_x) }
        self.lookup[0x35] = { self.op_and(self.am_dp_x) }
        ...
        self.lookup[0x0F] = { self.op_ora(self.am_abs_long) }
        self.lookup[0x17] = { self.op_ora(self.am_ind_long_dp_y) }
        self.lookup[0x19] = { self.op_ora(self.am_abs_y) }
        ...
        self.lookup[0x52] = { self.op_eor(self.am_ind) }
        self.lookup[0x57] = { self.op_eor(self.am_ind_long_dp_y) }
        self.lookup[0x5F] = { self.op_eor(self.am_abs_long) }
        self.lookup[0x4F] = { self.op_eor(self.am_abs_long) }

        self.lookup[0x89] = { self.op_bit_imm() }
        self.lookup[0x24] = { self.op_bit(self.am_dp) }
        self.lookup[0x2C] = { self.op_bit(self.am_abs) }
        self.lookup[0x34] = { self.op_bit(self.am_dp_x) }
        self.lookup[0x3C] = { self.op_bit(self.am_abs_x) }
        self.lookup[0x2F] = { self.op_bit(self.am_abs_long) }
        ...
        self.lookup[0x72] = { self.op_adc(self.am_ind) }
        self.lookup[0x77] = { self.op_adc(self.am_ind_long_dp_y) }
        self.lookup[0x75] = { self.op_adc(self.am_dp_x) }
        self.lookup[0x65] = { self.op_adc(self.am_dp) }
        self.lookup[0x7D] = { self.op_adc(self.am_abs_x) }
        self.lookup[0x79] = { self.op_adc(self.am_abs_y) }
        ...
        self.lookup[0xF5] = { self.op_sbc(self.am_dp_x) }
        self.lookup[0xF9] = { self.op_sbc(self.am_abs_y) }
        self.lookup[0xE5] = { self.op_sbc(self.am_dp) }
        self.lookup[0xFD] = { self.op_sbc(self.am_abs_x) }
        ...
        self.lookup[0xC5] = { self.op_cmp(self.am_dp) }
        self.lookup[0xD5] = { self.op_cmp(self.am_dp_x) }
        self.lookup[0xD9] = { self.op_cmp(self.am_abs_y) }
        self.lookup[0xCF] = { self.op_cmp(self.am_abs_long) }
        ...
        self.lookup[0x76] = { self.op_ror(self.am_dp_x, acc: false) }
        self.lookup[0x7E] = { self.op_ror(self.am_abs_x, acc: false) }

        self.lookup[0xE6] = { self.op_inc_dec(self.am_dp, inc: true) }
        self.lookup[0xEE] = { self.op_inc_dec(self.am_abs, inc: true) }
        self.lookup[0xD6] = { self.op_inc_dec(self.am_dp_x, inc: false) }
        self.lookup[0xD2] = { self.op_inc_dec(self.am_dp, inc: false) }
        self.lookup[0xDE] = { self.op_inc_dec(self.am_abs_x, inc: false) }
        self.lookup[0xCE] = { self.op_inc_dec(self.am_abs, inc: false) }
        self.lookup[0xC6] = { self.op_inc_dec(self.am_dp, inc: false) }
        self.lookup[0xE7] = { self.op_inc_dec(self.am_ind_long_dp, inc: true) }
        self.lookup[0xFE] = { self.op_inc_dec(self.am_abs_x, inc: true) }

        self.lookup[0x1A] = { self.op_inc_acc(); }
        self.lookup[0x3A] = { self.op_dec_acc(); }
        self.lookup[0xE8] = { self.op_inc_dec_reg(reg: &self.X, is8: self.e_mode || self.P.contains(.x)); self.cyclesRemaining = 2 }
        self.lookup[0xC8] = { self.op_inc_dec_reg(reg: &self.Y, is8: self.e_mode || self.P.contains(.x)); self.cyclesRemaining = 2 }
        self.lookup[0xCA] = { self.op_inc_dec_reg(reg: &self.X, is8: self.e_mode || self.P.contains(.x), inc: false); self.cyclesRemaining = 2 }
        self.lookup[0x88] = { self.op_inc_dec_reg(reg: &self.Y, is8: self.e_mode || self.P.contains(.x), inc: false); self.cyclesRemaining = 2 }

        self.lookup[0x5B] = { self.op_tcd() }
        self.lookup[0x1B] = { self.op_tcs() }
        self.lookup[0x7B] = { self.op_tdc() }
        self.lookup[0x3B] = { self.op_tsc() }
        self.lookup[0x8A] = { self.op_txa() }
        self.lookup[0x98] = { self.op_tya() }
        self.lookup[0xAA] = { self.op_tax() }
        self.lookup[0xA8] = { self.op_tay() }
        self.lookup[0x9A] = { self.op_txs() }
        self.lookup[0xBA] = { self.op_tsx() }
        self.lookup[0xBB] = { self.op_txy() }
        self.lookup[0x5A] = { self.op_tyx() }

        self.lookup[0x48] = { self.op_pha() }
        self.lookup[0x68] = { self.op_pla() }
        self.lookup[0xDA] = { self.op_phx() }
        self.lookup[0xFA] = { self.op_plx() }
        self.lookup[0x5A] = { self.op_phy() }
        self.lookup[0x7A] = { self.op_ply() }
        self.lookup[0x0B] = { self.op_phd() }
        self.lookup[0x2B] = { self.op_pld() }
        self.lookup[0x8B] = { self.op_phb() }
        self.lookup[0xAB] = { self.op_plb() }
        self.lookup[0x08] = { self.op_php() }
        self.lookup[0x28] = { self.op_plp() }
        self.lookup[0x4B] = { self.op_phk() }
        self.lookup[0x32] = { self.op_pea_imm16() }
        self.lookup[0xF4] = { self.op_pea_imm16() }
        self.lookup[0xD4] = { self.op_pei(self.am_dp) }
        self.lookup[0x62] = { self.op_pea_imm16() }

        self.lookup[0x4C] = { self.am_abs(); self.PC = UInt16(self.opAddr & 0xFFFF); self.cyclesRemaining += 3 }
        self.lookup[0x5C] = { self.am_abs_long(); self.PB = UInt8((self.opAddr >> 16) & 0xFF); self.PC = UInt16(self.opAddr & 0xFFFF); self.cyclesRemaining += 4 }
        self.lookup[0x6C] = { self.op_jmp_abs_ind() }
        self.lookup[0x7C] = { self.op_jmp_abs_ind_x() }
        self.lookup[0xDC] = { self.am_ind_long(); self.PB = UInt8((self.opAddr >> 16) & 0xFF); self.PC = UInt16(self.opAddr & 0xFFFF); self.cyclesRemaining += 6 }
        self.lookup[0x20] = { self.op_jsr() }
        self.lookup[0x22] = { self.op_jsl() }
        self.lookup[0x60] = { self.op_rts() }
        self.lookup[0x6B] = { self.op_rtl() }
        self.lookup[0x40] = { self.op_rti() }
        self.lookup[0x82] = { self.branchLong(self.am_abs_long) }
        self.lookup[0xFC] = { self.am_abs_ind_x(); self.pushWord(self.PC &- 1); self.PC = UInt16(self.opAddr & 0xFFFF); self.cyclesRemaining += 4 }

        self.lookup[0x80] = { self.branch(true) }
        self.lookup[0x90] = { self.branch(!self.P.contains(.c)) }
        self.lookup[0xB0] = { self.branch(self.P.contains(.c)) }
        self.lookup[0xD0] = { self.branch(!self.P.contains(.z)) }
        self.lookup[0xF0] = { self.branch(self.P.contains(.z)) }
        self.lookup[0x10] = { self.branch(!self.P.contains(.n)) }
        self.lookup[0x30] = { self.branch(self.P.contains(.n)) }
        self.lookup[0x50] = { self.branch(!self.P.contains(.v)) }
        self.lookup[0x70] = { self.branch(self.P.contains(.v)) }

        self.lookup[0x04] = { self.op_tsb(self.am_dp) }
        self.lookup[0x0C] = { self.op_tsb(self.am_abs) }
        self.lookup[0x14] = { self.op_trb(self.am_dp) }
        self.lookup[0x1C] = { self.op_tsb(self.am_abs_x) }
        self.lookup[0x14] = { self.op_trb(self.am_dp_x) }
